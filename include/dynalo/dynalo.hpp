...
inline handle invalid_handle() { return detail::native::invalid_handle(); }

namespace name {

/// @return The name prefix of a shared library on the current system
inline std::string prefix() { return detail::native::name::prefix(); }

/// @return The name suffix of a shared library on the current system
inline std::string suffix() { return detail::native::name::suffix(); }

/// @return The file extension of a shared library on the current system
inline std::string extension() { return detail::native::name::extension(); }
...
inline std::string to_native_name(const std::string& lib_name) {
...
inline native::handle open(const std::string& dyn_lib_path) { return detail::open(dyn_lib_path); }
...
inline void close(native::handle lib_handle) { detail::close(lib_handle); }
...
class library {
 ...
 public:
  ...
  library(library&& other) : m_handle(other.m_handle) { other.m_handle = native::invalid_handle(); }
  ...
  explicit library(const std::string& dyn_lib_path) : m_handle(dynalo::open(dyn_lib_path)) {}
  ...
  native::handle get_native_handle() { return m_handle; }
};
